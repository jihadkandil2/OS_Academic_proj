...
uint32 get_hard_limit()
{
	return curenv->rlimit;
}
int sys_get_perms_made(uint32 virtual_address)
{
...
void* sys_sbrk(int increment)
{
	//TODO: [PROJECT'23.MS2 - #08] [2] USER HEAP - Block Allocator - sys_sbrk() [Kernel Side]
	 ...
	 uint32 hard_limit = env->rlimit;
	 uint32 old_break = env->brk;
	 uint32 new_break ;
	if (increment == 0)
	{
		env->brk = old_break;
	...
	if (increment > 0)
	{
		...
		{
			uint32 *ptr;
			uint32 page_table = get_page_table(ptr_page_directory, virtual_address, &ptr);
			if (page_table == TABLE_NOT_EXIST )
			{
				create_page_table(ptr_page_directory,virtual_address);
			}

			allocate_user_mem(env , virtual_address, size_new_break);
			env->brk = new_break ;
			return (void*)old_break;
		}
		else
...
void* sys_sbrk(int increment)
{
	...
	struct Env* env=curenv;
				    ...
				    if (increment > 0)
							{
				    	        ...
				    	        uint32 size_new_break = (uint32)increment;
				    			new_break = ROUNDUP(old_break + size_new_break,PAGE_SIZE);
				    	        uint32 size_rounded =ROUNDUP(increment, PAGE_SIZE);
								uint32 virtual_address = old_break;

								if(new_break <= hard_limit)
								{
									env->brk = new_break ;
									allocate_user_mem(env , virtual_address, size_rounded );
									return (void*)old_break;
								}
								else
								{
				    ...
				    {
							uint32 size_new_break = (uint32)increment;
							new_break = old_break + size_new_break;
...
uint32 syscall(uint32 syscallno, uint32 a1, uint32 a2, uint32 a3, uint32 a4, uint32 a5)
{
	...
	case SYS_free_user_mem:
					...
					break;
	case SYS_hard_limit:
					    return get_hard_limit();
					    break;
	case SYS_PERMS_FOR_MALLOC:
			return sys_get_perms_made(a1);
		    break;
	//=====================================================================
	case SYS_cputs:
