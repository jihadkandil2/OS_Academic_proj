...
void fault_handler(struct Trapframe *tf)
{
	...
	else
	{
		if (userTrap)
		{
			...
			// start from user limit  or kernel base
			if( ((page_permissions & PERM_USER)== 0) && ((page_permissions & PERM_PRESENT)==1) && (fault_va>=USER_LIMIT) )
			{
				//cprintf("11111111111111111111111111111111\n\n");
			  sched_kill_env(curenv->env_id);
			}
			if((page_permissions & PERM_WRITEABLE)==0&&((page_permissions & PERM_PRESENT)==1))// ANY PAGE CONTAINS READ ONLY PERMISION writable lw tl3 b zero reed only
			{
				//cprintf("222222222222222222222222222222222222222222222\n\n");
			  sched_kill_env(curenv->env_id);   // access page write to it which unauthorized
			}
			if((fault_va >= USER_HEAP_START && fault_va<USER_HEAP_MAX) &&((page_permissions & PERM_AVAILABLE)==0) )
			{//check if this unmarked
			//	cprintf("3333333333333333333333333333333333333333333333\n\n");
				sched_kill_env(curenv->env_id);
			}
			/*============================================================================================*/
		}
