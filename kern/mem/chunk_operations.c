...
void allocate_user_mem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	uint32 n_of_pages=size/PAGE_SIZE;

	uint32* ptr_page_table ;
	uint32 bound= virtual_address+PAGE_SIZE*n_of_pages;
	for (uint32 i=virtual_address; i<bound; i+=PAGE_SIZE)
	{
		 int ret = get_page_table(e->env_page_directory, i, &ptr_page_table);
		 if (ret==TABLE_NOT_EXIST)//there is no page table for this page.
		 {
			 ptr_page_table= create_page_table(e->env_page_directory,i );
		 }
			pt_set_page_permissions(e->env_page_directory, i, PERM_AVAILABLE,0);
			ptr_page_table[PTX(i)] |= (PERM_AVAILABLE);
	}
...
void free_user_mem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	uint32 n_of_pages= ROUNDUP(size ,PAGE_SIZE)/PAGE_SIZE;
	uint32* ptr_page_table  ;
	uint32 bound= virtual_address + PAGE_SIZE * n_of_pages;
	uint32 va = virtual_address;

	   for( uint32 i = va ; i< (va + PAGE_SIZE *n_of_pages); i+=PAGE_SIZE)// loop by num of pages
	   {
		  // cprintf("\ni will remove add = %x \n\n", i );
		   //env_page_ws_print(e );
		   pt_set_page_permissions(e->env_page_directory, i , 0, PERM_AVAILABLE); // unmark بخلي بيت ب 0
...
/*
	   uint32 v = LIST_FIRST(&(e->page_WS_list))->virtual_address ;
	   while(v != e->page_last_WS_element->virtual_address)
	   {

		   struct WorkingSetElement *newElement = env_page_ws_list_create_element(e, v);
		  // unmap_frame(e->env_page_directory,  v);
		   env_page_ws_invalidate(e,  v);
		   LIST_INSERT_TAIL(&(e->page_WS_list), newElement);
		   v= LIST_FIRST(&(e->page_WS_list))->virtual_address ;
	   }
*/
	// Write your code here, remove the panic and write your code
