...
void env_set_nice(struct Env* e, int nice_value)
{
	...
	//Comment the following line
    //panic("Not implemented yet");
	e->nice = nice_value;//to set nice value
	fixed_point_t load_mul2=fix_scale(load_avg,(int)2);//2*load_avg
	...
	fixed_point_t multiply=fix_mul(division,e->recent_cpu);//2*load_avg/2*load_avg+1*recent
	fixed_point_t fixed_recent=fix_add(multiply,fix_int((int)e->nice));//2*load_avg/2*load_avg+1*recent+nice
	fixed_point_t recent_over4=fix_div(fixed_recent,fix_int((int)4));//recent/4
	fixed_point_t nice_mul2=fix_mul(fix_int((int)e->nice),fix_int((int)2));//nice*2
	fixed_point_t pri_sub_recent=fix_sub(fix_int((int)PRI_MAX),recent_over4);//PRI_MAX-[recent/4]
	fixed_point_t priority=fix_sub(pri_sub_recent,nice_mul2);//PRI_MAX-[recent/4]-[nice*2]
//	e->priority=fix_trunc(priority);
...
int env_get_recent_cpu(struct Env* e)
{
	...
	//Comment the following line
    //panic("Not implemented yet");
	fixed_point_t fixed_100=fix_int((int)100);
	fixed_point_t fixed_100mulrecent=fix_mul(e->recent_cpu,fixed_100);
	return fix_round(fixed_100mulrecent);
	//return 100*fix_round(fixed_recent);//to convert fixed point to the nearest integer
	return 0;
}
int get_load_average()
{
    ...
    //panic("Not implemented yet");
	fixed_point_t fixed_100 = fix_int((int)100);
	fixed_point_t fixed_100mulload=fix_mul(load_avg,fixed_100);
	return fix_round(fixed_100mulload);
    //return 100*fix_round(fixed_load);//to convert fixed point to the nearest integer
	return 0;
}
/********* for BSD Priority Scheduler *************/
//==================================================================================//
