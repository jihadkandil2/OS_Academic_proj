...
int process_command(int number_of_arguments, char** arguments)
{
	   //int strncmp(const char *p, const char *q, uint32 n)
		// check on validity of name of command

		char *sended_command =arguments[0];              //clk
		int entered_Arguments = number_of_arguments - 1 ;
		char *in_commands ;

  	  LIST_INIT(&foundCommands);
		for (int i=0 ; i <NUM_OF_COMMANDS ; i++)
		{
			if (strcmp(commands[i].name ,sended_command) == 0 )       //command name found
			{
                  if ((commands[i].num_of_args == -1 && entered_Arguments >= 1)|| entered_Arguments == commands[i].num_of_args)
                  {
                	  return i ;
                  }
				  else                                                // same name but diff number of arguments
				  {
			      ...
			      char* array_command =commands[i].name;
			      bool is_sub =true ;
			      int counter =0 ;
			      for (int k =0 ; k < strlen(sended_command) ; k++)  // c l k
			      {
			    	  for (int j = 0 ; j< strlen(array_command); j++) // n c l o c k
			    	  {
			    		  if (sended_command[k] == array_command[j])
			    		  {
			    			  k++;
			    			  counter++;
			    			  if (counter== strlen(sended_command))
			    			  {
			    				  break;
			    			  }
			    		  }
		...
		if (LIST_SIZE(&foundCommands) == 0)        // not exist at all
		{
			return CMD_INVALID ;
		}
		else {                                     // there is list
